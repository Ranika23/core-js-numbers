...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const acos = Math.acos(scalar / (modul1 * modul2));
  return acos;
}
...
function toNumber(value, def) {
  ...
  if (typeof value !== 'object' && value !== null) {
    if (Number(value) > -9999 && Number(value) < 9999) {
      result = Number(value);
    } else {
      result = def;
    }
...
function getCountOfOddNumbers(number) {
  let result;
  if (number % 2 === 0) {
    result = Math.abs(Math.trunc(number / 2));
  } else if (number % 2 !== 0 && number >= 0) {
    result = Math.abs(Math.trunc(number / 2) + 1);
  } else if (number % 2 !== 0 && number < 0) {
    result = Math.abs(Math.trunc(number / 2) - 1);
  }
  return result;
